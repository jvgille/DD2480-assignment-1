...
public class TestLIC4 {
	...
	public void test1() { //Test the whichQuadrant function for corner and normal cases
		...
		for(int i = 0; i < x.length; ++i) {
			indexes[i] = d.whichQuadrants(x[i],y[i]);
	...
	public void test2() { //Test the LIC4 function for corner case
		...
		assertFalse(d.LIC4()); //Cannot be true since we need points in three different quadrants but we only have 2 points
	...
	public void test3() { //Test the LIC4 function for corner case
		...
		d.Y = y;
		d.PARAMETERS = p;
		assertTrue(d.LIC4()); //Should be true since the first point is in quadrant 1 and the second is in quadrant 2
	}
	...
	public void test4() { //Test the LIC4 function on regular array (answer should be false)
		...
		d.Y = y;
		d.PARAMETERS = p;
		assertFalse(d.LIC4()); //Should be false since we don't have 4 consecutive points lying in more than 3 quadrants
	}
	...
	public void test5() { //Test the LIC4 function on regular array (answer should be true)
		...
		d.Y = y;
		d.PARAMETERS = p;
		assertTrue(d.LIC4()); //Should be true since the last four point belongs to quadrants respectively 1, 2, 3, and 4
	}
